module vga_controller (
  ...
  localparam [2:0] s0 = 000, s1 = 001, s2 = 010, s3 = 011;
  ...
  always @(posedge vga_clk or negedge rst_n) begin
    ...
    if (~rst_n) begin
      v_counter <= 0;
      h_counter <= 0;
    end else if (h_counter == 10'd799) begin
  ...
  always @(*) begin
    ...
    case (h_cs)
      ...
      s0: begin
        if (h_counter == 'd639) h_ns = s1;
        else h_ns = h_cs;
      end

      s1: begin
        if (h_counter == 'd655) h_ns = s2;
        else h_ns = h_cs;
      end

      s2: begin
        if (h_counter == 'd751) h_ns = s3;
        else h_ns = h_cs;
      end

      s3: begin
        if (h_counter == 'd799) h_ns = s0;
        else h_ns = h_cs;
      end
  ...
  always @(*) begin
    ...
    case (v_cs)
      ...
      s0: begin
        if (v_counter == 'd479) v_ns = s1;
        else v_ns = v_cs;
      end

      s1: begin
        if (v_counter == 'd489) v_ns = s2;
        else v_ns = v_cs;
      end

      s2: begin
        if (v_counter == 'd491) v_ns = s3;
        else v_ns = v_cs;
      end

      s3: begin
        if (v_counter == 'd524) v_ns = s0;
        else v_ns = v_cs;
      end
  ...
  always @(*) begin : out_block2

    if (v_cs == s2) v_sync = 0;
    else v_sync = 1;


    if (h_cs == s0 && v_cs == s0) v_on = 1;
    else v_on = 0;
    // VGA Blank
    vga_blank_N = v_on;
